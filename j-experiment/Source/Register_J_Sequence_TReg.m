...
for i = 1:n
    ...
    subplot(2,3,i+1);
%     dataT = dataTs(:, :, i);
%     viewImage(dataT, omega(1:3), m(1:3), 'title', 'Rotated Image');
end

% Combine the reference images
dataRs = cat(3, dataR, dataR, dataR, dataR, dataR);
...
%     
%     % Save all the plots
%     FolderName = 'Research/j-experiment/Plots/J_MSIR';
%     FigList = findobj(allchild(0), 'flat', 'Type', 'figure');
%     for iFig = 1:length(FigList)
%       FigHandle = FigList(iFig);
%       FigName   = strcat(strcat(strcat('J_MSIR_', num2str(get(FigHandle, 'Number'))), '_'), num2str(i));
